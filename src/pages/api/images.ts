  ...
  logging(req, res, async (req, res) => {
    if (req.method === 'POST') {
      ...
      form.parse(req, async (err, fields, files) => {
        ...
        const imagePromises = (Array.isArray(files.images) ? files.images : [files.images]).map(
          image => {
            const filename = `${Date.now()}_${image.name}`;
            const imagePath = `${IMAGE_PATH}/${filename}`;
            return fs
              .writeFile(imagePath, image.data)
              .then(() => prisma.image.create({ data: { path: `${filename}` } }));
          }
        );
        const images = await Promise.all(imagePromises);
        const tagPromises = (Array.isArray(tagIds) ? tagIds : [tagIds]).map(async tagId => {
